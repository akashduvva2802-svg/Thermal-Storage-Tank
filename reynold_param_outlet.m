...
Reh_array = [2.17e3, 1.73e4, 4.32e4, 6.48e4];
...
for k = 1:length(Reh_array)
    ...
    if k == 1
        Y_ref = 0.0364325.*X.^3 - 0.27987.*X.^2 + 0.810238.*X - 0.00773461;
    elseif k == 2
        Y_ref = 0.0220459.*X.^3 - 0.183495.*X.^2 + 0.554106.*X - 0.0207826;
    elseif k == 3
        Y_ref = 0.0164966.*X.^3 - 0.127926.*X.^2 + 0.347748.*X - 0.0178729;
    elseif k == 4
        Y_ref = -0.00269993.*X.^4 + 0.0304894.*X.^3 - 0.135446.*X.^2 + 0.287779.*X - 0.0212621;
    end
    ...
    fprintf('Re_c = %e : Correlation (R) = %f, R^2 = %f, MSE = %e\n', Reh_array(k), R_val, R_sq, MSE_val);
end
...
lgd = legend(h_sim, {['Re_c = 2.17 \times 10^3'], ...
              ['Re_c = 1.73 \times 10^4'], ...
              ['Re_c = 4.32 \times 10^4'], ...
              ['Re_c = 6.48 \times 10^4']});
